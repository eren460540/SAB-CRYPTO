...
@client.tree.command(name="profile", description="Check your balance")
async def profile(interaction: discord.Interaction, user: discord.Member = None):
    ...
    holdings = "\n".join([f"{COINS[k]['emoji']} **{k}:** {v:,.4f}" for k,v in p['portfolio'].items() if v > 0])
    embed.add_field(name="Holdings", value=holdings or "*Empty*", inline=False)
    await interaction.response.send_message(embed=embed)
...
async def buy(interaction: discord.Interaction, coin: str, amount: float):
    ...
    if coin not in COINS or amount <= 0:
        return await interaction.response.send_message("❌ Invalid request.", ephemeral=True)
    ...
    if p['sab_balance'] < cost:
        return await interaction.response.send_message(f"❌ **Nah Uh.** Need {cost:,.2f} {SAB_EMOJI}.", ephemeral=True)
    ...
    await interaction.response.send_message(f"✅ Bought {amount} {coin}!")
...
async def deposit(interaction: discord.Interaction):
    ...
    await chan.send(f"{interaction.user.mention} Welcome to the bank. Provide proof of deposit here.")

@client.tree.command(name="add_sab", description="Admin Only")
...
async def add_sab(interaction: discord.Interaction, user: discord.Member, amount: float):
    ...
    await interaction.response.send_message(f"✅ Added {amount} {SAB_EMOJI} to {user.name}")
